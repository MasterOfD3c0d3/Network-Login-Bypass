...
do
	case "$1" in
		...
		-i | --interface)
			...
			inet=`ifconfig "$interface" | grep "inet "`
			if [ -z "$inet" ]; then
				clear
				echo -e $title
				echo -e $div
...
if [[ $EUID -ne 0 ]]; then
	echo -n $(basename "$0")
	echo -e ": This script needs root access!"
	exit 1
fi
...
gateway=`route -n | grep UG | cut -d " " -f 10`
...
#PROGRESS BAR CODE
num=`wc -l < tempList_network_login_bypass.txt`
porc=$(awk "BEGIN {printf \"%.4f\",50/${num}}")
load=0
...
while read line; do
	...
	#PROGRESS BAR CODE
	load=$(awk "BEGIN {printf \"%.4f\",${load}+(${porc}*2)}")
	intLoad=${load%.*}
	position=$(($intLoad/2))
	...
	echo "Please, wait..."
	echo
	echo "Checked IPs from range:"
	
	hash=$(printf '#%.0s' $(seq 1 $position))
	rest=$((50-$position))
	if [ ! $rest = 0  ]; then
		point=$(printf '.%.0s' $(seq 1 $rest))
	else
		point=''
	fi
	
	echo -e "\033[5D[$hash$point] $intLoad%"
